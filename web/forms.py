...
class SignupForm(forms.ModelForm):
	...
	def __init__(self, *args, **kwargs):
		...
		if 'email' in settings.WEB3AUTH_USER_SIGNUP_FIELDS:
			self.fields['email'].required = True
		self.fields[settings.WEB3AUTH_USER_ADDRESS_FIELD].required = True

	def clean_address_field(self):
		validate_eth_address(self.cleaned_data[settings.WEB3AUTH_USER_ADDRESS_FIELD])
		return self.cleaned_data[settings.WEB3AUTH_USER_ADDRESS_FIELD].lower()

	class Meta:
		model = get_user_model()
		fields = signup_fields
		labels = {
			'username': _('Tao 0x Address'),
		}
		help_texts = {
			'username': _(''),
		}
...
setattr(SignupForm, 'clean_' + settings.WEB3AUTH_USER_ADDRESS_FIELD, SignupForm.clean_address_field)
